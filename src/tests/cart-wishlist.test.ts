...
import { prismaMock, adminMock, verifyIdTokenMock, MOCK_USER, MOCK_PRODUCT, MOCK_CART_ITEM, MOCK_WISHLIST_ITEM } from "./setup.js";
...
describe("GET /api/cart", () => {
  it("200 — returns user cart", async () => {
...
describe("DELETE /api/cart/:productId", () => {
  it("401 — rejects unauthenticated", async () => {
    const res = await request(app).delete(`/api/cart/${MOCK_PRODUCT.id}`);
    expect(res.status).toBe(401);
  });
});
...
describe("POST /api/wishlist", () => {
  ...
  it("401 — rejects unauthenticated", async () => {
    const res = await request(app).post("/api/wishlist").send({ productId: MOCK_PRODUCT.id });
    expect(res.status).toBe(401);
  });
});
...
describe("DELETE /api/wishlist/:productId", () => {
  it("200 — removes item", async () => {
    prismaMock.wishlistItem.findUnique.mockResolvedValue(MOCK_WISHLIST_ITEM);
